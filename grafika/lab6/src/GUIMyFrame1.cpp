...
{
	int gradient = 0;
	gradient += image.GetRed(x - 1, y - 1) * -1;
	gradient += image.GetRed(x - 1, y) * -1;
	gradient += image.GetRed(x - 1, y + 1) * -1;
	gradient += image.GetRed(x + 1, y - 1) * 1;
	gradient += image.GetRed(x + 1, y) * 1;
	gradient += image.GetRed(x + 1, y + 1) * 1;
	gradient += image.GetRed(x,y) * 0;
	gradient += image.GetRed(x,y+1) * 0;
	gradient += image.GetRed(x,y-1) * 0;
	return gradient;
}
...
void GUIMyFrame1::m_b_prewitt_click( wxCommandEvent& event )
{
 // TO DO: Pionowa maska Prewitta
	...
	for (int y = 1; y < Img_Cpy.GetHeight() - 1; y++)
	{
		...
		{
			int gradient = yGradient(Img_Org, x, y);
			if (gradient < 0)
				gradient = 0;
			if (gradient > 255)
				gradient = 255;
			data[3 * (y * Img_Cpy.GetWidth() + x) + 0] = gradient;
			data[3 * (y * Img_Cpy.GetWidth() + x) + 1] = gradient;
			data[3 * (y * Img_Cpy.GetWidth() + x) + 2] = gradient;
		}
	}	
}
